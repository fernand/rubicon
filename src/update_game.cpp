...
void create_game_config(char *screenshot_path, GameConfig *config)
{
    ...
    {
        uint8_t num_virt_cells = Board::kVirtCellsForDepth[depth];
        ...
        for (uint8_t i = 0; i < num_virt_cells; i++)
        {
            ...
            if (is_real_cell)
            {
                ...
                config->all_cells[real_cell_idx] = Cell{cell_idx};
                real_cell_idx += 1;
            }
            cell_idx++;
            current_x += DST;
...
void update_player_cells(Board *board, const uint8_t *buffer, size_t y_stride)
{
    ...
    for (uint8_t depth = 1; depth <= MAX_DEPTH; depth++)
    {
        ...
        for (uint8_t i = 0; i < num_virt_cells; i++)
        {
            ...
            {
                if (!board->cell_occupied(kAI, cell))
                {
                    board->occupy_cell(kAI, cell);
                    printf("%u changed to ai\n", cell.idx);
                }
            }
            else if (buffer[offset] == 255 && buffer[offset + 1] == 0 && buffer[offset + 2] == 255)
            {
                if (!board->cell_occupied(kPlayer, cell))
                {
                    board->occupy_cell(kPlayer, cell);
    ...
    uint8_t num_ai_cells = board->count_occupied_cells(kAI);
    uint8_t num_player_cells = board->count_occupied_cells(kPlayer);
    if (num_ai_cells > num_player_cells)
    {
        board->set_player_to_play(kAI);
        board->set_round_to_play(Round{(u8)(num_player_cells % 6 + 1)});
    }
    else
    {
        board->set_player_to_play(kPlayer);
        board->set_round_to_play(Round{(u8)(num_ai_cells % 6 + 1)});
    }
}
