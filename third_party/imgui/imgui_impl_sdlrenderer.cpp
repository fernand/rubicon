...
static void ImGui_ImplSDLRenderer_SetupRenderState()
{
    ...
    // FIXME: Technically speaking there are lots of other things we could backup/setup/restore during our render
    // process.
...
void ImGui_ImplSDLRenderer_RenderDrawData(ImDrawData *draw_data)
{
    ...
    // If the user has specified a scale factor to SDL_Renderer already via SDL_RenderSetScale(), SDL will scale
    // whatever we pass to SDL_RenderGeometryRaw() by that scale factor. In that case we don't want to be also scaling
    // it ourselves here.
    float rsx = 1.0f;
    float rsy = 1.0f;
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                const float *xy =
                    (const float *)((const char *)(vtx_buffer + pcmd->VtxOffset) + IM_OFFSETOF(ImDrawVert, pos));
                const float *uv =
                    (const float *)((const char *)(vtx_buffer + pcmd->VtxOffset) + IM_OFFSETOF(ImDrawVert, uv));
#if SDL_VERSION_ATLEAST(2, 0, 19)
                const SDL_Color *color = (const SDL_Color *)((const char *)(vtx_buffer + pcmd->VtxOffset) +
                                                             IM_OFFSETOF(ImDrawVert, col)); // SDL 2.0.19+
#else
                const int *color = (const int *)((const char *)(vtx_buffer + pcmd->VtxOffset) +
                                                 IM_OFFSETOF(ImDrawVert, col)); // SDL 2.0.17 and 2.0.18
#endif

                // Bind texture, Draw
                SDL_Texture *tex = (SDL_Texture *)pcmd->GetTexID();
                SDL_RenderGeometryRaw(bd->SDLRenderer, tex, xy, (int)sizeof(ImDrawVert), color, (int)sizeof(ImDrawVert),
                                      uv, (int)sizeof(ImDrawVert), cmd_list->VtxBuffer.Size - pcmd->VtxOffset,
...
bool ImGui_ImplSDLRenderer_CreateFontsTexture()
{
    ...
    bd->FontTexture =
        SDL_CreateTexture(bd->SDLRenderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STATIC, width, height);
